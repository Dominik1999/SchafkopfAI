...
class SchafkopfEnv:
    ...
    def step(self, action, action_prob=1):
        ...
        if self.gamestate.game_stage == Rules.BIDDING:
            ...
            if self.gamestate.bidding_round[self.gamestate.current_player] is not None:
                self.gamestate.game_player, self.gamestate.game_type = self.rules.highest_game(
                    self.gamestate.bidding_round, self.gamestate.first_player)
            ...
            self.gamestate.current_player = (self.gamestate.current_player + 1) % 4
            if self.gamestate.contra[self.gamestate.current_player] is not None:
            ...
            self.gamestate.current_player = (self.gamestate.current_player + 1) % 4
            if self.gamestate.retour[self.gamestate.current_player] is not None:
                self.gamestate.game_stage = Rules.TRICK

        elif self.gamestate.game_stage == Rules.TRICK:

            self.gamestate.course_of_game_playerwise[self.gamestate.trick_number][
                self.gamestate.current_player] = action
            self.gamestate.course_of_game[self.gamestate.trick_number][self.gamestate.played_cards % 4] = action
            self.gamestate.action_probabilities[3 + self.gamestate.trick_number][
                self.gamestate.current_player] = action_prob
            ...
            if self.gamestate.played_cards % 4 == 0:  # trick complete
                if self.gamestate.trick_number == 0:
                    first_player = self.gamestate.first_player
                else:
                    first_player = self.gamestate.trick_owner[self.gamestate.trick_number - 1]
                ...
                if self.gamestate.game_type[1] == 0:  # Sauspiel
                    if self.gamestate.trick_number == 0:
                        first_player_of_trick = self.gamestate.first_player
                    else:
                        first_player_of_trick = self.gamestate.trick_owner[self.gamestate.trick_number - 1]
                    card_played = trick_cards[first_player_of_trick]
                    rufsau = [self.gamestate.game_type[0], 7]
                    if self.gamestate.game_type[0] == card_played[
                        0] and card_played != rufsau and card_played not in self.rules.get_sorted_trumps(
                        self.gamestate.game_type) and rufsau in self.player_cards[first_player_of_trick]:
                        self.gamestate.davongelaufen = first_player_of_trick

                self.gamestate.trick_number += 1

        state["game_state"] = self.gamestate
        state["allowed_actions"] = self.rules.allowed_actions(self.gamestate,
                                                              self.player_cards[self.gamestate.current_player])
        state["current_player_cards"] = self.player_cards[self.gamestate.current_player]
        self.last_allowed_actions = state["allowed_actions"]
    ...
    def print_game(self):
        ...
        for i in range(4):
            ...
            if self.gamestate.bidding_round[i] is None:
                ...
                if self.gamestate.bidding_round[i][1] is not None:
                    if self.gamestate.bidding_round[i][0] is not None:
        ...
        if self.gamestate.game_type[1] is not None:
            if self.gamestate.game_type[0] is not None:
                ...
                for player in range(4):
                    ...
                    if self.gamestate.course_of_game_playerwise[trick][player] == [None, None]:
                        ...
                        trick_str_ += self.rules.card_color[
                                          self.gamestate.course_of_game_playerwise[trick][player][0]] + " " + \
