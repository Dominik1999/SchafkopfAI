...
def main():
    ...
    for j in Spieler:
        print("%s: %s " %(Spieler[j].name, ", ".join([str(karte) for karte in Spieler[j].karten])))
    ...
    if Spieler_Punkte > 60:
        print("Spieler (%s, %s) gewinnen mit %d Punkten" % (
            spielende_spieler[0].name,
            spielende_spieler[1].name,
            Spieler_Punkte
        ))
    else:
        print("Nicht-Spieler (%s, %s) gewinnen mit %d Punkten" % (
